...
where position[i] is the starting mile of the ith car 
...
def car_fleet(target: int, position: list[int], speed: list[int]) -> int:
  # pair up the values so it's easier to calculate
  # create a stack
  # iterate from the reverse the sorted values
  pairs = [[p, s] for p, s in zip(position, speed)]
  stack = []

  for p, s in sorted(pairs)[::-1]:
    # add to the stack the time
    stack.append((target - p) / s)
    if len(stack) >= 2 and stack[-1] <= stack[-2]:
      stack.pop()
    # if the time is less than the item on the stack before, pop off the stack
